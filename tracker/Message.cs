    ...
    class Message
    {
        ...
        public byte[] toByte()
        {
            switch (this.msgType)
            {
                ...
                case MessageType.Start: return constructMessage(MessageType.Start, this.peerID, this.roomID);
        ...
        {
            if (msgType != MessageType.Create) return null;
            else
            {
                byte[] roomIDByte = Encoding.ASCII.GetBytes(roomID);
                Array.Resize(ref roomIDByte, GameConstant.roomIDSize);
